...
var ReportTo = "文件传输助手";//修改这里即修改像谁发1，请把此参数改为你要发1的人，用于自己检查自己是否已经上报，不建议去除此功能.
...
function checkPage()
{
    ...
    else if(sendflag == false & (text("最常使用").exists() || text("联系人").exists() || text("群聊").exists()|| text("功能").exists() || text("关注的公众号").exists()) & (desc("清除").exists())) 
    {
        pageId = 0x04;
    }
    else if (text("校园卡").exists() & desc("消息").exists() & desc("设置").exists()) 
    {
        pageId = 0x05;
    }
